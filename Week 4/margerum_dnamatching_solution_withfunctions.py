def add_indel(DNAstr):
    ...
    #else:
        #DNAstr == DNAstr2
        #DNAstr2 = DNAstr2[:index] + '-' + DNAstr2[index:]
        #DNAstr2 == mDNAstr
...
def delete_indel():
    ...
    if string_choice == '1':
        #This loop ensures that the index is not out of range
        while index > len(DNA_str_1) or index < 0:
            print ('Error. Index out of range.')
            index = int(raw_input('At what index: '))
            
        #This loop ensures that the character at index is
        #an indel
        while DNA_str_1[index] != '-':
        ...
        #starting at index+1.   
        DNA_str_1 = DNA_str_1[:index] + DNA_str_1[index + 1:]
    ...
    if string_choice == '2':
        ...
        #This loop ensures that the character at index is
        #an indel
        while DNA_str_2[index] != '-':
            print ('Error. Cannot delete a character that is not an indel.')
            index = int(raw_input('At what index: '))
            
        #deletes an indel at the index by concatenating string 2
        #upto postion index-1 and the rest of the string
        #starting at index+1.
        DNA_str_2 = DNA_str_2[:index] + DNA_str_2[index + 1:]
...
, q(quit): ')
    ...
    if action == 'a':
        ...
        if string_choice == '1':
            ...
            DNA_str_2 = add_indel(DNA_str_2) #got a pointer from Natasha here
    else:
        if action == 'd':
            delete_indel()
        else:
